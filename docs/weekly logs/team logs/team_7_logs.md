...
This cycle, the focus has been to complete the last pages required for the instructor and student views. Work on the admin view has started as well. The goal is to get all the core functionalities done by July 17th. Most of the backend endpoints are complete, and the last main components which need to be worked on are the OMR integration and admin views.
